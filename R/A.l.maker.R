  ...
  if (family == "survival") {
    ...
    if (basehaz == "pwexp") {
      ...
      for (i in which(status == 1)) {  # Loop over events only
        ...
        exp_eta <- exp(Z[risk_set, ] %*% beta_hat)
        # Calculate the weighted sum of covariates
        weighted_covariates <- sweep(Z[risk_set, , drop = FALSE], 1, exp_eta, "*")
        # Weighted average of Z
        weighted_cov <- colSums(weighted_covariates) / sum(exp_eta)
        # Weighted covariance matrix of Z in the risk set
        weighted_cov_matrix <- t(Z[risk_set, , drop = FALSE]) %*% sweep(Z[risk_set, , drop = FALSE], 1, exp_eta, "*") / sum(exp_eta)
