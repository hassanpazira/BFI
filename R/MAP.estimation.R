...
MAP.estimation <- function(y, X, family = c("gaussian", "binomial", "survival"),
  ...
  if ((!is.null(treatment)) & is.null(gamma_bfi) & is.null(RCT_propens)) { # or treat_round=="first"
  ...
  if (family %in% c("binomial", "gaussian")) {
    ...
    if ((!is.null(treatment)) & is.null(gamma_bfi) & is.null(RCT_propens)) {
      ...
      y <- as.vector(X_treat_old[, wich_treat])
    }
    ...
    if ((!is.null(treatment)) & ((!is.null(gamma_bfi)) | (!is.null(RCT_propens)))) {
      ...
      X_treat_old <- X
      wich_treat <- which(colnames(X) == treatment)
      #if (is.null(gamma_bfi) & is.null(RCT_propens)) X <- X[, -wich_treat]
      Z_treat <- as.vector(X_treat_old[, wich_treat])
      X <- X[, -wich_treat]
      p <- ncol(X)
      ...
      if ((!is.null(gamma_bfi)) | (!is.null(RCT_propens))) {
        if (missing(refer_treat)) refer_treat <- levels(factor(Z_treat))[1]
        Zli <- relevel(factor(Z_treat), ref = refer_treat)  # Set refer_treat as reference.
        ...
        if (!is.null(gamma_bfi)) {
          if (length(gamma_bfi) != dim(X)[2])
            stop("The length of 'gamma_bfi' should be equal to the number of covariates.",
          ...
          if (is.null(names(gamma_bfi))) {
            ...
            stop("'gamma_bfi' should have names. names(gamma_bfi) should not be 'NULL'.")
          }
          if (!identical(names(gamma_bfi), colnames(X)))
  ...
  if (family == "survival") {
    ...
    if (!is.null(treatment)) {
      ...
      if ((!is.null(gamma_bfi)) | (!is.null(RCT_propens))) {
        ...
        if (!is.null(gamma_bfi)) {
          if (length(gamma_bfi) != dim(X_without_treat)[2])
            stop("The length of 'gamma_bfi' should be equal to the number of covariates.",
                 "'gamma_bfi' should consist of regression coefficients only, excluding nuisance parameters.")
          if (!identical(names(gamma_bfi), colnames(X_without_treat)))
            stop("names(gamma_bfi) and X[, -which(colnames(X) == treatment)] differ in elements, order, or both.")
          beta_bfi <- gamma_bfi
