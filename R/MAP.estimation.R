...
MAP.estimation <- function(y, X, family = c("gaussian", "binomial", "survival"),
                           ...
                           treatment = NULL, refer_treat, coef_bfi = NULL, RCT_propens = NULL,
                           initial = NULL, alpha = 0.1, max_order = 2, n_intervals = 4,
                           min_max_times, center_zero_sample = FALSE,
  ...
  if (family %in% c("binomial", "gaussian")) {
    ...
    if (!is.null(treatment)) {
      if ((!is.null(coef_bfi)) & (!is.null(RCT_propens)))
        stop("When 'treatment' is not 'NULL', one of 'RCT_propens' or 'coef_bfi' should be 'NULL'. ",
             "In the first round, both should be 'NULL'.")
      if (length(treatment) > 1)
        stop("Only one covariate for treatment.")
      # if (is.numeric(treatment)) {
      #   if (intercept == TRUE) { # X is with 'intercept'.
      #     treatment <- colnames(X)[treatment+1]
      #   } else { # X is without 'intercept'.
      #     treatment <- colnames(X)[treatment]
      #   }
      # }
      ...
      #if (is.null(coef_bfi) & is.null(RCT_propens)) X <- X[, -which(colnames(X) == treatment)]
      X_treat_old <- X
      X <- X[, -which(colnames(X) == treatment)]
      p <- ncol(X)
      Lambda <- Lambda[-which(colnames(Lambda) == treatment), -which(colnames(Lambda) == treatment)]
      if ((!is.null(coef_bfi)) | (!is.null(RCT_propens))) {
        if (missing(refer_treat)) refer_treat <- levels(factor(X_treat_old[, treatment]))[1]
        Zli <- relevel(factor(X_treat_old[, treatment]), ref = refer_treat)  # Set refer_treat as reference.
        ...
        if (!is.null(coef_bfi)) {
          if (length(coef_bfi) != dim(X)[2])
            stop("The length of 'coef_bfi' should be equal to the number of covariates.",
                 "'coef_bfi' should consist of regression coefficients only, excluding nuisance parameters.")
          if (is.null(names(coef_bfi))) {
            if (!is.null(colnames(coef_bfi))) names(coef_bfi) <- colnames(coef_bfi)
            stop("'coef_bfi' should have names. names(coef_bfi) should not be 'NULL'.")
          }
          if (!identical(names(coef_bfi), colnames(X)))
            stop("names(coef_bfi) and X[, -which(colnames(X) == treatment)] differ in elements, order, or both.")
          beta_bfi <- coef_bfi
          eli <- exp(as.numeric(X %*% beta_bfi))/(1 + exp(as.numeric(X %*% beta_bfi)))
        }
        ...
        for_ATE <- c(m_1, m_2, sum_z_y, sum_z_y2, sum_z_e, sum_z_y_e, sum_1_z_1_e, sum_1_z_y_1_e)
        propensity <- eli
      } else {
        for_ATE <- NULL
        propensity <- NULL
        if (missing(refer_treat)) refer_treat <- NULL
      }
    } else {
      coef_bfi <- NULL
      RCT_propens <- NULL
    ...
    if (family == "binomial") {
      ...
      theta_optim <- try(optim(initial_beta_sig, fn = negloglik.theta,
                               gr = NULL, y = y, X = X, Lambda = Lambda,
                               family = family, lower = rep(-Inf, p),
    ...
    if (family == "gaussian") {
      if (is.null(initial)) {
        initial_beta_sig <- c(rep(0, p+1))
      } else {
        if (length(initial) == (p + 1) & (inherits(initial, "numeric") |
                                          inherits(initial, "integer"))) {
          initial_beta_sig <- initial
        } else {
          stop(paste("'initial' should be a numerical vector of length",
      ...
      if (length(initial_beta_sig) != dim(Lambda)[1])
        stop("Dim. of 'Lambda' is ", dim(Lambda)[1],"x",dim(Lambda)[2],
             ", while the length of 'theta' is ",length(initial_beta_sig),".")
      theta_optim <- try(optim(initial_beta_sig,
                               fn = negloglik.theta, gr = NULL,
                               y = y, X = X, Lambda = Lambda, family = family,
                               lower = c(rep(-Inf, p+1)),
                               upper = rep(Inf, p+1), method = "L-BFGS-B",
                               control = control), TRUE)
  ...
  if (family == "survival") {
    ...
    if (!is.null(treatment)) {
      if (basehaz != "unspecified") stop("When 'treatment' is not 'NULL', the baseline hazard should be 'unspecified'.")
      if ((!is.null(coef_bfi)) & (!is.null(RCT_propens)))
        stop("When 'treatment' is not 'NULL', one of 'RCT_propens' or 'coef_bfi' should be 'NULL'. ",
             "In the first round, both should be 'NULL'.")
      if (length(treatment) > 1)
        stop("Only one covariate for treatment.")
      # if (is.numeric(treatment)) treatment <- colnames(X)[treatment]  #! or X?
      if (!is.character(treatment)) stop("Covariate '", treatment,"' should be a character.")
      if (nlevels(factor(X[, treatment])) != 2)
        stop("Covariate '", treatment,"' must have only two categories, e.g., 0 and 1.")
      X_with_treat <- X
      X_without_treat <- X[, -which(colnames(X) == treatment)]
      X <- X_without_treat
      p <- ncol(X)
      Lambda_treat <- Lambda[which(colnames(Lambda) == treatment), which(colnames(Lambda) == treatment)]
      if (!is.matrix(Lambda_treat)) Lambda_treat <- as.matrix(Lambda_treat)
      Lambda <- Lambda[-which(colnames(Lambda) == treatment), -which(colnames(Lambda) == treatment)]
      if ((!is.null(coef_bfi)) | (!is.null(RCT_propens))) {
        if (missing(refer_treat)) refer_treat <- levels(factor(X_with_treat[, treatment]))[1]
        Zli <- relevel(factor(X_with_treat[, treatment]), ref = refer_treat)  # Set refer_treat as reference.
        Zli_0_1 <- as.numeric(Zli) - 1  # Convert to 0/1, so that 0 is reference.
        if (length(Zli_0_1) != nrow(X)) # It is not necessary!
          stop("The length of treatment should be equal to the number of individuals, n_l.")
        if (!is.null(coef_bfi)) {
          if (length(coef_bfi) != dim(X_without_treat)[2])
            stop("The length of 'coef_bfi' should be equal to the number of covariates.",
                 "'coef_bfi' should consist of regression coefficients only, excluding nuisance parameters.")
          if (!identical(names(coef_bfi), colnames(X_without_treat)))
            stop("names(coef_bfi) and X[, -which(colnames(X) == treatment)] differ in elements, order, or both.")
          beta_bfi <- coef_bfi
          eli <- exp(as.numeric(X_without_treat %*% beta_bfi)) /
            (1 + exp(as.numeric(X_without_treat %*% beta_bfi)))
        }
        if (!is.null(RCT_propens)) {
          if (length(RCT_propens) != nrow(X))
            stop("Length of 'RCT_propens' should be n_l.")
        ...
        if (length(eli) != nrow(X))
          stop("The length of Propensity Scores should be equal to the number of individuals, n_l.")

        ## Optimizations for Zeta
        if (basehaz == "unspecified") {
          if (is.null(initial)) {
            initial_beta_omega <- c(rep(0.0, 1))
          } else {
            if (length(initial)==ncol(X_with_treat) &
                (inherits(initial, "numeric") | inherits(initial, "integer"))) {
              initial_beta_omega <- initial[which(colnames(X_with_treat) == treatment)]
            } else {
              stop(paste("'initial' should be a numerical vector of length",
                         sQuote(ncol(X_with_treat)),"."))
            }
          }
          if (length(initial_beta_omega) != NCOL(Lambda_treat))
            stop("Dim. of 'Lambda' is ", dim(Lambda_treat)[1],"x",dim(Lambda_treat)[2],
                 ", while the length of 'theta' is ",length(initial_beta_omega),".")
          zeta_optim <- optim.survival(initial_beta_loga_logb = initial_beta_omega,
                                        q_l=q_l, tps=tps, y=y, X = as.data.frame(Zli_0_1), Lambda = Lambda_treat,
                                        family = family, basehaz = basehaz, wli = 1/eli, control = control)
          if (!is.null(attr(zeta_optim, "class")) | inherits(zeta_optim, "try-error")) {
            # conv <- FALSE
            stop("The algorithm did not converge. Change 'initial' values.")
          }
          if (zeta_optim$convergence == 0) {
            conv <- 0
          } # paste("The algorithm has converged.")
          if (zeta_optim$convergence == 1) {
            conv <- 1
          } # paste("The iteration limit 'maxit' had been reached.")
          if (zeta_optim$convergence %in% c(51, 52)) {
            conv <- noquote(paste(" 2  (Message from the 'L-BFGS-B' method: ''",
                                  zeta_optim$message, "'')"))
          }
          zeta_hat <- zeta_optim$par
          value_zata_hat <- zeta_optim$value
          ## A_hat_zeta: curvature matrix estimator (variance)
          A_hat_zeta <- A.l.maker(y = y, X = as.data.frame(Zli_0_1), Lambda = Lambda_treat, family = family,
                             theta_hat = zeta_hat, q_l = q_l, tps=tps, basehaz = basehaz, wli=1/eli)
          names(zeta_hat) <- colnames(A_hat_zeta)
          if (any(diag(solve(as.matrix(A_hat_zeta))) < 0))
            stop("In this center, some diagonal elements of the inverse curvature ",
                 "matrix are negative! Increase the sample size, e.g., > 50.")
          ## sd_A_zeta
          sd_A_zeta <- sqrt(diag(solve(as.matrix(A_hat_zeta))))

          for_ATE <-c(zeta_hat, A_hat_zeta, sd_A_zeta, as.integer(zeta_optim$convergence))
          names(for_ATE) <- c("zeta_hat", "variance_zeta", "sd_zeta", "convergence")
          propensity <- eli
        }
      } else {
        if (missing(refer_treat)) refer_treat <- NULL
    ...
    if (basehaz == "poly") {
      ...
      for (ql in q_l:max_order) {
        ...
        theta_for_M_ql <- optim.survival(initial_beta_loga_logb=initial_beta_omega,
                                    ...
                                    family=family, basehaz=basehaz, control = control)
        ...
        # A_hat
        A_l_maker_new <- A.l.maker(y=y, X=X, Lambda=Gamma_dot, q_l=ql, tps=tps, basehaz=basehaz,
                                   family=family, theta_hat=theta_for_M_ql$par)
      ...
      theta_optim <- optim.survival(initial_beta_loga_logb=initial_beta_omega,
                                    ...
                                    family=family, basehaz=basehaz, wli=wli, control = control)
      if (!is.null(attr(theta_optim, "class")) | inherits(theta_optim, "try-error")) {
        # conv <- FALSE
        stop("The algorithm did not converge. Change 'initial' values.")
      }
      ...
      ## A_hat: curvature matrix estimator
      A_hat <- A.l.maker(y = y, X = X, Lambda = Lambda, family = family,
                         theta_hat = theta_hat, q_l = q_l, tps=tps, basehaz = basehaz, wli=wli)
      names(theta_hat) <- colnames(A_hat)
      ...
      theta_optim <- optim.survival(initial_beta_loga_logb=initial_beta_omega,
                               ...
                               basehaz = basehaz, control = control)
      if (!is.null(attr(theta_optim, "class")) | inherits(theta_optim, "try-error")) {
        # conv <- FALSE
        stop("The algorithm did not converge. Change 'initial' values.")
      }
      ...
      ## A_hat: curvature matrix estimator
      A_hat <- A.l.maker(y = y, X = X, Lambda = Lambda, family = family,
                         theta_hat = theta_hat, q_l = q_l, tps = tps, basehaz = basehaz)
      names(theta_hat) <- colnames(A_hat)
      if (any(diag(solve(as.matrix(A_hat))) < 0))
        stop("In this center, some diagonal elements of the inverse curvature ",
             "matrix are negative! Increase the sample size, e.g., > 50.")
    ...
    if (basehaz == "poly") {
      ...
      if (!is.null(treatment)) {
        output <- list(
          ...
          treatment = treatment, refer_treat = refer_treat, coef_bfi = coef_bfi,
