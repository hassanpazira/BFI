...
MAP.estimation <- function(y, X, family=gaussian, Lambda, intercept=TRUE, initial=NULL, control=list()){
  ...
  if (is.null(colnames(X))) {
    ...
    if (c("Intercept") %in% colnames(X)) stop("'intercept' should be the first column of 'X'.")
  }
  ...
  if (intercept == F) {
    X <- X[,-1, drop=F]
    if (c("Intercept") %in% colnames(X) | c("(Intercept)") %in% colnames(X))
  ...
  output <- list(theta_hat=theta_hat, A_hat=A_hat, sd=sd_A, Lambda=Lambda, value=value_theta_hat,
                 family=family$family, converged=conv, control=control)
  return(output)
}
