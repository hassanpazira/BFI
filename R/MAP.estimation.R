...
MAP.estimation <- function(y, X, family = c("gaussian", "binomial", "survival"),
                           Lambda, intercept = TRUE, initial = NULL,
                           basehaz = c("weibul", "exp", "gomp", "poly", "pwexp"),
                           alpha = 0.1, max_order = 2, n_intervals = 4,
  ...
  if (family == "survival") {
    ...
    const_vars <- sqrt(xvar) == 0
    if (any(const_vars)) {
      stop("Predictor(s) ", paste0(colnames(X)[which(const_vars)],sep=" "), "has zero variance!",
    ...
    if (basehaz == "poly") {
      ...
      for (ql in q_l:max_order) {
        ...
        # theta_for_M_ql <- try(optim.bfi(initial_beta_loga_logb=initial_beta_omega,
        #                                 q_l=ql, tps=tps, y=y, X=X, Lambda=Gamma_dot,
        #                                 family=family, basehaz=basehaz), TRUE)
        theta_for_M_ql <- optim.bfi(initial_beta_loga_logb=initial_beta_omega,
                                    q_l=ql, tps=tps, y=y, X=X, Lambda=Gamma_dot,
                                    family=family, basehaz=basehaz)
        if(!is.null(attr(theta_for_M_ql, "class")) |
           inherits(theta_for_M_ql, "try-error")) {
          initial_beta_omega  <- c(rep(0.5, p+ql),-.5) # should be chosen by users!
          theta_for_M_ql <- optim.bfi(initial_beta_loga_logb=initial_beta_omega,
                                      q_l=ql, tps=tps, y=y, X=X, Lambda=Gamma_dot,
                                      family=family, basehaz=basehaz)
          if(!is.null(attr(theta_for_M_ql, "class")) |
             inherits(theta_for_M_ql, "try-error")) {
            initial_beta_omega  <- c(rep(1, p+ql), -1) # should be chosen by users!
            theta_for_M_ql <- optim.bfi(initial_beta_loga_logb=initial_beta_omega,
                                        q_l=ql, tps=tps, y=y, X=X, Lambda=Gamma_dot,
                                        family=family, basehaz=basehaz)
            if(!is.null(attr(theta_for_M_ql, "class")) |
               inherits(theta_for_M_ql, "try-error"))
              stop("Change initial values.")
          }
        }
        theta_M_ql[1:(p+ql+1),ql+1,1] <- theta_for_M_ql$par
        # A_hat
        A_l_maker_new <- A.l.maker(y=y, X=X, Lambda=Gamma_dot, q_l=ql, tps=tps,
                                   basehaz=basehaz, family=family, theta_hat=theta_for_M_ql$par)
      ...
      if (center_zero_sample == TRUE) {
        ...
        if (length(levels(factor(X_old[, zero_sample_cov]))) < 2)
      ...
      if (length(initial_beta_omega) != dim(Lambda)[1])
        stop("Dim. of 'Lambda' is ", dim(Lambda)[1],"x",dim(Lambda)[2],
             ", while the length of 'theta' is ",length(initial_beta_omega),".")
      theta_optim <- optim.bfi(initial_beta_loga_logb=initial_beta_omega,
  ...
  if (family == "survival") {
    ...
    if (basehaz == "poly") {
      output <- list(
        ...
        zero_sample_cov = zero_sample_cov, refer_cat = refer_cat,
