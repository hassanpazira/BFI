...
MAP.estimation <- function(y, X, family=gaussian, Lambda, intercept=TRUE,
  ...
  if (intercept == F) {
    ...
    if (any(c("Intercept", "(Intercept)") %in% colnames(X)))
      stop("'intercept' should be the first column of 'X'.")
    if (any(Lambda[,1] != 0))
      stop("Since 'intercept=FALSE', the first row and column of Lambda should be all zeros.")
    Lambda_orig <- Lambda
    Lambda <- Lambda[-1,-1]
  } else {
  ...
  if (any(const_vars)) {
    if (intercept==T) {
      stop("Predictor(s) ", paste0(colnames(X[,-1, drop=F])[which(const_vars)],sep=" "),
           "has zero variance! Try the 'bfi()' function to handle this variable.")
    } else {
  ...
  if (family$family=="binomial") {
    if (intercept==T) {
      if (dim(Lambda)[1] != p) stop(paste("'Lambda' in this family should have p =",sQuote(p),"dimensions", "\n",
  ...
  if (family$family=="gaussian") {
    if (intercept==T) {
      if (dim(Lambda)[1] != (p+1)) stop(paste("'Lambda' should have p+1 =",sQuote(p+1),"dimensions", "\n",
  ...
  output <- list(theta_hat=theta_hat, A_hat=A_hat, sd=sd_A, Lambda=Lambda, formula=formula1,
                 n=n, np=p, value=value_theta_hat, family=family$family, intercept=intercept,
