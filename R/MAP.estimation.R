...
MAP.estimation <- function(y, X, family = c("gaussian", "binomial", "survival"),
  ...
  if (family %in% c("binomial", "gaussian")) {
    ...
    if ((!is.null(treatment)) & !is.null(treat_round) & ((!is.null(gamma_bfi)) | (!is.null(RCT_propens)))) {
      if (treat_round=="second" & (is.null(gamma_bfi)) & (is.null(RCT_propens)))
      ...
      if (!is.null(gamma_bfi))
        gamma_bfi <- as.data.frame(gamma_bfi)
      X_no_treat <- X
      if (intercept == TRUE) {
        X <- cbind(X[, 1], Z_only_treat)
      ...
      # if ((!is.null(gamma_bfi)) | (!is.null(RCT_propens))) { # can be deleted!
    ...
    if (family == "binomial") {
      ...
      theta_optim <- try(optim(initial_beta_sig, fn = negloglik.theta,
                               ...
                               family = family, wli = wli, lower = rep(-Inf, p),
      ...
      if ((!is.null(gamma_bfi)) | (!is.null(RCT_propens))) {
        ...
        if (basehaz == "unspecified") {
          ...
          zeta_optim <- optim.survival(initial_beta_loga_logb = initial_beta_omega,
                                       q_l=q_l, tps=tps, y=y, X = as.data.frame(Zli_0_1),
                                       Lambda = Lambda_treat, family = family,
                                       basehaz = basehaz, wli = wli, control = control)
          ...
          A_hat_zeta <- A.l.maker(y = y, X = as.data.frame(Zli_0_1), Lambda = Lambda_treat,
                                  family = family, theta_hat = zeta_hat, q_l = q_l,
                                  tps=tps, basehaz = basehaz, wli=wli)
