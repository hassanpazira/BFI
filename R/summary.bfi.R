...
summary.bfi <- function(object, cur_mat = FALSE,
  ...
  if (object$family == c("gaussian")) {
    ...
    # If is.null(object$stratified)==T means the 'object' is from MAP.estimation().
  ...
  if (is.null(object$stratified)) { # means the object is from MAP.estimation()
    if (object$family == c("survival")) {
    ...
    if (object$family %in% c("binomial","gaussian")) {
      if (!is.null(object$ATE)) {
        ...
        cat("\n   IPTW: ", format(object$ATE$IPTW, digits = digits), "\n")
        cat("  wIPTW: ", format(object$ATE$wIPTW, digits = digits), "\n")
      }
