...
bfi <- function(theta_hats = NULL, A_hats, Lambda,
  ...
  if (!is.null(treat_round)) {
    ...
    if (treat_round == "second" & is.null(for_ATE)) stop("In the second round, 'for_ATE' must be a list.")
  ...
  if (stratified == FALSE) {
    ...
    if (!is.null(for_ATE)) {
      ...
      if (family %in% c("gaussian", "binomial")) {
        ...
        N <- sum_over_L[1] + sum_over_L[2]
        ATE1 <- (sum_over_L[6] - sum_over_L[8]) / N
        ATE2 <- (sum_over_L[6]/sum_over_L[5]) - (sum_over_L[8]/sum_over_L[7])
        ATE <- list(method_1=ATE1, method_2=ATE2)
      }
      if (family == "survival") {
        if (basehaz != "unspecified") stop("Only 'unspecified' baseline hazard can be used.")
        sum_over_L <- Reduce("+", for_ATE)
        A_bfi_zeta <- sum_over_L[2] + Lambda_all_zeta[[L + 1]] - Reduce("+", Lambda_all_zeta[seq_len(L)])
        sd_bfi_zeta <- sqrt(diag(solve(as.matrix(A_bfi_zeta))))
        zeta_hat_bfi <- t(solve(as.matrix(A_bfi_zeta)) %*%
                            Reduce("+", Map(function(x, y) x %*% y,
                                            sum_over_L[2], sum_over_L[1])))
        ATE <- list(zeta_BFI = zeta_hat_bfi, A_zeta_BFI = A_bfi_zeta, sd_zeta_BFI = sd_bfi_zeta)
      }
    } else {
      ATE <- NULL
    }
  ...
  output <- list(theta_hat = theta_hat_bfi, A_hat = A_bfi, sd = sd_bfi, family = family,
                 basehaz = basehaz, stratified = stratified, strat_par = old_strat_par, ATE = ATE)
  class(output) <- "bfi"
  return(output)
}
