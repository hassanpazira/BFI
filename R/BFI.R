...
bfi <- function(theta_hats=NULL, A_hats, Lambda, stratified=FALSE, strat_par=NULL, center_spec=NULL) {
  ...
  if (stratified == T & !is.null(strat_par)) {
  ...
  if (stratified == F) {
    ...
    A_bfi <- Reduce("+", A_hats) + Lambda_all[[L+1]] - Reduce("+", Lambda_all[seq_len(L)])
    ...
    if (last_Lam_dim == dim(Lambda_all[[1]])[1]) {
      if (all(colnames(Lambda_all[[1]])==colnames(Lambda_all[[L+1]]))) {
        ...
        if (length(strat_par) == 1) {
          if (1 %in% strat_par) {
            if (colnames(Lambda_all[[1]])[ncol(Lambda_all[[1]])] != colnames(Lambda_all[[L+1]])[ncol(Lambda_all[[L+1]])]) {
    ...
    if (is.null(center_spec)) {
      if (length(strat_par) == 1) {
        ...
        noncore <- new_noncore <- cbind(seq_len(L), (last_Lam_dim - L + 1):last_Lam_dim)
      ...
      for (j in seq_len(L)) {
        if (j==1) {
          ...
          Lambda_a_l[L+1] <- list(Lambda_all[[L+1]][-as.numeric(noncore),-as.numeric(noncore),drop=F])
          A_hats_b_l <- list(A_hats[[j]][strat_par,strat_par,drop=F])
          Lambda_b_l <- list(Lambda_all[[j]][strat_par,strat_par,drop=F])
          Lambda_b_l[L+1] <- list(Lambda_all[[L+1]][as.numeric(noncore),as.numeric(noncore),drop=F])
          A_hats_tilda_b_l <- list(A_hats_b_l[[j]] + Lambda_b_l[[L+1]][new_noncore[j,],new_noncore[j,],drop=F] -
                                   Lambda_b_l[[j]])
          ...
          A_hats_tilda_b_l[j] <- list(A_hats_b_l[[j]] + Lambda_b_l[[L+1]][new_noncore[j,],new_noncore[j,],drop=F] -
                                      Lambda_b_l[[j]])
      ...
      part2 <- Map("%*%", lapply(seq_len(L), function(x) t(A_hats_ab_l[[x]])), theta_hat_a_l_a_bfi)
      ...
      if (K == L)
        stop("The number of levels of 'center_spec' should NOT be equal to the number of centers. Otherwise, ",
             "use 'stratified = T', 'center_spec = NULL' but set 'strat_par' to 1, 2 or 1:2.")
      strat_par <- c(1) # For now, we assume 'strat_par' is 'intercept'. However, the input 'strat_par' is NULL.
      if (length(strat_par) == 1) {
        ...
        strat_par <- c(1, length(theta_hats[[1]]))
        noncore <- new_noncore <- cbind(lev_cen, (last_Lam_dim - K + 1):last_Lam_dim)
        new_noncore[, 2] <- noncore[,1] + K
      }
      if (names(theta_hats[[1]])[1] != "(Intercept)" & (1 %in% strat_par)) {
        stop("It seems 'intercept = F' for centers, while in the current version of the package ",
             "the center specific covariate is only possible for 'intercept'!")
      }
      for (j in seq_len(L)) {
        if (j==1) {
          A_hats_1a_l <- list(A_hats[[j]][-strat_par,-strat_par,drop=F])
          Lambda_1a_l <- list(Lambda_all[[j]][-strat_par,-strat_par,drop=F])
          Lambda_1a_l[L+1] <- list(Lambda_all[[L+1]][-as.numeric(noncore),-as.numeric(noncore),drop=F])
          A_hats_1b_l <- list(A_hats[[j]][strat_par,strat_par,drop=F])
          Lambda_1b_l <- list(Lambda_all[[j]][strat_par,strat_par,drop=F])
          Lambda_1b_l[L+1] <- list(Lambda_all[[L+1]][as.numeric(noncore),as.numeric(noncore),drop=F])
          A_hats_1ab_l <- list(A_hats[[j]][-strat_par,strat_par,drop=F])
          theta_hat_1a_l <- list(theta_hats[[j]][-strat_par])
          theta_hat_1b_l <- list(theta_hats[[j]][strat_par])
        } else {
          A_hats_1a_l[j] <- list(A_hats[[j]][-strat_par,-strat_par,drop=F])
          Lambda_1a_l[j] <- list(Lambda_all[[j]][-strat_par,-strat_par,drop=F])
          A_hats_1b_l[j] <- list(A_hats[[j]][strat_par,strat_par,drop=F])
          Lambda_1b_l[j] <- list(Lambda_all[[j]][strat_par,strat_par,drop=F])
          A_hats_1ab_l[j] <- list(A_hats[[j]][-strat_par,strat_par,drop=F])
      ...
      if (dim(A_hats_1a_l[[1]])[1] != dim(Lambda_1a_l[[L+1]])[1]) {
        stop("Check the dimension of the last matrix in Lambda. It should be changed.")
      }
      A_1bl_thet1bl <- Map("%*%", A_hats_1b_l , theta_hat_1b_l)
      A_1abl_thet1al <- Map("%*%", lapply(seq_len(L), function(x) t(A_hats_1ab_l[[x]])) , theta_hat_1a_l)
      for (k in seq_len(K)) {
        z_l_k <- which(center_spec == lev_cen[k])
        if (k==1) {
          which_lev <- which(lev_cen[k] == lev_cen) # should be edited if 'strat_par' is not 'intercept'
          A_hats_1bk <- list(Reduce("+", A_hats_1b_l[z_l_k]) + Lambda_1b_l[[L+1]][which_lev,which_lev] -
          ...
          which_lev <- which(lev_cen[k] == lev_cen) # should be edited if 'strat_par' is not 'intercept'
          A_hats_1bk[k] <- list(Reduce("+", A_hats_1b_l[z_l_k]) + Lambda_1b_l[[L+1]][which_lev,which_lev] -
                                  Reduce("+", Lambda_1b_l[z_l_k]))#!
      ...
      A_1a_bfi <- Reduce("+", A_hats_1a_l) + Lambda_1a_l[[L+1]] - Reduce("+", Lambda_1a_l[seq_len(L)])
      A_hat_ab2 <- Map("%*%", A_hats_1abk , A_hats_1bk_inv)
      A_hat_ab3 <- Map("%*%", A_hat_ab2, lapply(seq_len(K), function(x) t(A_hats_1abk[[x]])))
      ...
      part1 <- Map("%*%", lapply(seq_len(K), function(x) t(A_hats_1abk[[x]])), theta_hat_1a_bfi_list)
      part2 <- Map("-", map02 , part1)
      theta_hat_1bk_bfi <- Map("%*%", A_hats_1bk_inv, part2)
      name_nuis <- names(theta_hats[[1]])[strat_par]
      nam_non_nuis <- names(theta_hats[[1]])[-strat_par]
      theta_nuis <- matrix(unlist(theta_hat_1bk_bfi), ncol = length(strat_par), byrow = TRUE)
      colnames(theta_nuis) <- name_nuis
      vec_theta_alls <- new_names <- NULL
      kk <- 0
      for (k in seq_len(n_par)) {
      ...
      for (k in seq_len(K)) {
        ...
        A_bfi[noncore[k,], -noncore] <- A_bfi[-noncore, noncore[k,]] <- A_hats_1abk[[k]]
      }
      sd_bfi <- sqrt(diag(solve(as.matrix(A_bfi))))
    }
  }
