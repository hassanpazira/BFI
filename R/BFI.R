...
bfi <- function(theta_hats = NULL, A_hats, Lambda,
                ...
                stratified = FALSE, strat_par = NULL, center_cluster = NULL,
                basehaz = c("weibul","exp","gomp","poly","pwexp","unspecified"),
                theta_A_polys = NULL, p, q_ls, center_zero_sample = FALSE,
  ...
  if (stratified == TRUE & is.null(strat_par) & is.null(center_cluster)) {
    stop("Since 'stratified = TRUE', only one of 'strat_par' or 'center_cluster'
         could be NULL.")
  }
  if (stratified == TRUE & !is.null(strat_par) & !is.null(center_cluster)) {
    stop("Since 'stratified = TRUE', only one of 'strat_par' or 'center_cluster'
  ...
  if (stratified == FALSE & (!is.null(strat_par) | !is.null(center_cluster))) {
    stop("Since 'stratified = FALSE', both 'strat_par' and 'center_cluster'
         sould be NULL.")
  }
  ...
  if (stratified == FALSE) {
    ...
    if (is.null(center_cluster)) {
      ...
      if (length(center_cluster) != L) {
        stop("Length of the vector 'center_cluster' should be equal to ", sQuote(L))
      }
      ...
      if (K == L) {
        stop(
          "The number of levels of 'center_cluster' should NOT be equal to the ",
      ...
      for (k in seq_len(K)) {
        z_l_k <- which(center_cluster == lev_cen[k])
        if (k == 1) {
