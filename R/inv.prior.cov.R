...
inv.prior.cov <- function(X, lambda = 1, L = 2L,
                          ...
                          strat_par = NULL, center_cluster = NULL,
  ...
  if (stratified == TRUE & is.null(strat_par) & is.null(center_cluster)) {
    stop("Since 'stratified = TRUE', only one of 'strat_par' or 'center_cluster' can be NULL.")
  }
  if (stratified == TRUE & !is.null(strat_par) & !is.null(center_cluster)) {
    stop("Since 'stratified = TRUE', only one of 'strat_par' or 'center_cluster' could be non-NULL.")
  }
  if (stratified == FALSE & (!is.null(strat_par) | !is.null(center_cluster))) {
    stop("Since 'stratified = FALSE', both 'strat_par' and 'center_cluster' should be NULL.")
  }
  ...
  if (stratified == FALSE) {
    ...
    if (is.null(center_cluster)) {
      ...
      if (length(center_cluster) != L) {
        stop(
          "Either the length of the vector 'center_cluster' should be equal to ",
          sQuote(L), " or L should be ", sQuote(length(center_cluster))
        )
      }
      if (!is.factor(center_cluster)) {
        center_cluster <- factor(center_cluster)
      }
      lev_cen <- levels(center_cluster)
      K <- length(lev_cen)
      if (K < 2) {
        stop("The number of levels of 'center_cluster' should be >= 2.")
      }
      if (K == L) {
        stop(
          "The number of levels of 'center_cluster' should NOT be equal to ",
          "the number of centers. Otherwise, use 'stratified = TRUE', ",
          "'center_cluster = NULL' but set 'strat_par' to 1, 2 or 1:2."
        )
      }
      strat_par <- 1
