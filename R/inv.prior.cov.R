...
inv.prior.cov <- function(X, lambda=1, L=2L, family=gaussian, intercept=TRUE, stratified=FALSE,
    ...
    if (is.null(center_spec)) {
      ...
      if (family$family == "gaussian") {
        ...
        if (intercept==T) {
          if (length(strat_par) == 1) {
            if (1 %in% strat_par) {
              rownames(Lambda) <- colnames(Lambda) <- c(paste0(name_Lambda[1], rep("_loc", L), seq_len(L)),
      ...
      if (family$family == "binomial") {
        ...
        Lambda <- diag(dia_lam, length(dia_lam))
        rownames(Lambda) <- colnames(Lambda) <- c(paste0(name_Lambda[1], rep("_loc", L), seq_len(L)), name_Lambda[-1])
      }
    } else {
      if (length(center_spec) != L)
        stop("Either the length of the vector 'center_spec' should be equal to ", sQuote(L),
             " or L should be ", sQuote(length(center_spec)))
      ...
      strat_par <- 1 # For now, we assume 'strat_par' is 'intercept'. However, the input 'strat_par' is NULL.
      if (length(strat_par) == 1) {
      ...
      if (family$family == "gaussian") {
        if (intercept == F) {
          if (length(strat_par) > 1 | 1 %in% strat_par) {
            stop("'intercept' can not ne 'FALSE' for centers, while in the current version of the package ",
        ...
        if (intercept==T) {
          if (length(strat_par) == 1) {
            if (1 %in% strat_par) {
              rownames(Lambda) <- colnames(Lambda) <- c(paste0(name_Lambda[1], rep("_", K), lev_cen),
                                                        name_Lambda[-c(1, length(name_Lambda))],
                                                        name_Lambda[length(name_Lambda)])
            }
            if (2 %in% strat_par) { # can not be possible for the current version!
              rownames(Lambda) <- colnames(Lambda) <- c(name_Lambda[-length(name_Lambda)],
      ...
      if (family$family == "binomial") {
        if (intercept == F) {
          stop("'intercept = F' for centers, while in the current version of the package ",
               "the center specific covariate is only possible for 'intercept'!")
        }
        if (!is.numeric(strat_par))
          stop("'strat_par' should be an integer")
        ...
        Lambda <- diag(dia_lam, length(dia_lam))
        rownames(Lambda) <- colnames(Lambda) <- c(paste0(name_Lambda[1], rep("_", K), lev_cen), name_Lambda[-1])
      }
    }
